...
import { CheckCircle2, FileText, LoaderCircle, Lock, Sparkles } from "lucide-react"
...
export function SowReupload({ projectId, currentSowUrl, projectStatus }: Props) {
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="rounded-[1.25rem] border border-border/70 bg-card p-4 space-y-4">
        ...
        {locked ? (
          <div className="flex items-center gap-2 rounded-2xl border border-border/50 bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
            <Lock className="size-4 shrink-0" />
            SOW re-extraction is disabled for closed projects.
          </div>
        ) : null}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Gemini will extract deliverables and credentials and append them to the
                      existing checklist.
                    </p>
                    <input
                      type="file"
